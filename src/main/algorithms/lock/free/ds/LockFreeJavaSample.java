 ...
 *
 */
public class LockFreeJavaSample {
	...
	public static void main(String[] args) throws InterruptedException {
        ...
        // standard stack implementation.
         ClassicStack<Integer> operStack = new ClassicStack<Integer>(); //Lock-Free Stack-9699069
        // definition.
		//LockFreeStack<Integer> operStack = new LockFreeStack<>();//22407738
		Random randomIntegerGenerator = new Random();
		for(int j=0;j<10;j++) {
			operStack.push(Integer.valueOf(randomIntegerGenerator.nextInt()));
		}
        ...
        for(int k=0;k<stackPushThreads;k++) {
        	Thread pushThread = new Thread(() -> {
        		System.out.println("Pushing into statck...");
        		while(true) {
        			operStack.push(Integer.valueOf(randomIntegerGenerator.nextInt()));
        		}
        	}); 
        	// making the threads low priority before starting them
	...
	private static class LockFreeStack<T>{
		...
		private AtomicReference<StackNode<T>> headNode = new AtomicReference<>();
		private AtomicInteger noOfOperations = new AtomicInteger(0);
		
		public int getNoOfOperations() {
			return this.noOfOperations.get();
		}
		...
		public void push(T value){
			...
			noOfOperations.getAndIncrement();
        }
